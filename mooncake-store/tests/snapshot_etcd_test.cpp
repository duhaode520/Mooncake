...
TEST_F(SnapshotEtcdTest, SnapshotPersistViaDaemon) {
    ...
    EXPECT_EQ(expected_meta_size, static_cast<uint64_t>(metadata_value.size()))
        << "Metadata size mismatch";
    EXPECT_EQ(expected_seg_size, static_cast<uint64_t>(segments_value.size()))
        << "Segments size mismatch";

    auto meta_crc =
        Crc32c(reinterpret_cast<const uint8_t*>(metadata_value.data()),
               metadata_value.size());
    auto seg_crc =
        Crc32c(reinterpret_cast<const uint8_t*>(segments_value.data()),
               segments_value.size());
...
//         snapshot_success.store(result.has_value(),
//         std::memory_order_relaxed); if (!result.has_value()) {
...
//         // Remove immediately to avoid exhausting the segment during the
//         loop. auto remove_result = service_->Remove(key);
...
//         << "Snapshot finished too quickly; simulated delay may not be
//         active";
//     EXPECT_LT(max_latency_ms, 500.0)
//         << "Master operations appear blocked during snapshot; max
//         latency(ms)="
//         << max_latency_ms;
// }
