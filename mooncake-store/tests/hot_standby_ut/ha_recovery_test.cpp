...
class HaRecoveryTest : public ::testing::Test {
   protected:
    ...
    bool LoadSnapshotAndRecover() {
        std::string snap_id;
        uint64_t snap_seq = 0;
        std::vector<std::pair<std::string, StandbyObjectMetadata>> snap_data;

        bool loaded = mock_snapshot_->LoadLatestSnapshot(
            "test_cluster", snap_id, snap_seq, snap_data);

        if (loaded) {
            for (const auto& [key, meta] : snap_data) {
                metadata_store_->PutMetadata(key, meta);
            }
            applier_->Recover(snap_seq);
        }
        return loaded;
    }
...
TEST_F(HaRecoveryTest, GC_BasicCleanupSemantics) {
    ...
    OpLogEntry entry;
    EXPECT_EQ(ErrorCode::ETCD_KEY_NOT_EXIST, mock_store_->ReadOpLog(14, entry));
