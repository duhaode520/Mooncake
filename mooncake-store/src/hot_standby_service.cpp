...
ErrorCode HotStandbyService::Start(const std::string& primary_address,
    ...
    if (config_.enable_snapshot_bootstrap && !snapshot_provider_) {
        ...
        LOG(INFO) << "Snapshot bootstrap enabled: created default "
                     "EtcdSnapshotProvider, root="
                  << kDefaultSnapshotRoot;
    }
#else
    if (config_.enable_snapshot_bootstrap && !snapshot_provider_) {
        LOG(WARNING)
            << "Snapshot bootstrap enabled but STORE_USE_ETCD is not set; "
    ...
    if (!has_local_state && config_.enable_snapshot_bootstrap) {
        if (!snapshot_provider_) {
            LOG(WARNING)
                << "Snapshot bootstrap enabled but snapshot_provider is null; "
    ...
    if (!has_local_state && config_.enable_snapshot_bootstrap &&
        ...
        if (loaded) {
            ...
            if (snapshot_seq_id == 0 && !snapshot.empty()) {
                LOG(ERROR)
                    << "Loaded non-empty snapshot but snapshot_seq_id is 0. "
                       "This will likely cause persistent missing seq_id "
                       "warnings. "
            ...
            LOG(WARNING) << "Snapshot bootstrap enabled but no snapshot is "
                            "available yet; "
                            "falling back to OpLog-only bootstrap (this may "
                            "fail if old OpLogs were GC'd)";
        }
    }
    ...
    // Start OpLogReplicator with a consistent "read then watch(from
    // revision+1)" sequence.  Retry with exponential backoff to avoid getting
    // stuck in RECONNECTING state where nothing drives recovery.
    static constexpr int kMaxStartRetries = 3;
    static constexpr int kStartRetryBaseMs = 500;
    bool replicator_started = false;
    if (!oplog_replicator_) {
        LOG(ERROR) << "OpLogReplicator is not available, cannot start";
        state_machine_.ProcessEvent(StandbyEvent::FATAL_ERROR);
        return ErrorCode::INTERNAL_ERROR;
    }
    for (int attempt = 0; attempt < kMaxStartRetries; ++attempt) {
        if (oplog_replicator_->StartFromSequenceId(last_applied_seq_id)) {
...
ErrorCode HotStandbyService::Promote() {
    ...
    // best-effort.
    // - Stop OpLogReplicator first to avoid concurrent Apply from watch
    // callbacks.
    if (oplog_replicator_) {
        oplog_replicator_->Stop();
    }
    ...
    // Stop replication (OpLogReplicator will stop).
