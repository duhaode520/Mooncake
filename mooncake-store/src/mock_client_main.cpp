...
DEFINE_int32(write_interval_ms, 1000,
   ...
   public:
    MockClientSimulator(const std::string& master_address)
        ...
        LOG(INFO) << "  max_keys: " << FLAGS_max_keys
                  << " (ignored, keys will grow unlimited)";
        ...
        if (FLAGS_mount_segment) {
            ...
            if (mount_result.has_value()) {
                ...
                LOG(INFO)
                    << "[Mount] Sending initial ping to register client...";
                auto ping_result = client_.Ping();
                if (ping_result.has_value()) {
                    LOG(INFO)
                        << "[Mount] Initial ping SUCCESS - client registered";
                } else {
            ...
            LOG(INFO)
                << "[Mount] Segment mounting disabled (--nomount_segment)";
            LOG(INFO)
                << "[Mount] PutStart operations may fail if no segment is "
                   "available";
        }
    }
    ...
    void Run() {
        ...
        std::thread ping_thread([this]() {
            ...
            // Send first ping IMMEDIATELY (no sleep) to ensure client is
            // registered This compensates for any delay between constructor and
            // Run() call
            LOG(INFO) << "[Ping] Sending immediate ping to register client...";
            auto first_ping_result = client_.Ping();
            if (first_ping_result.has_value()) {
                LOG(INFO)
                    << "[Ping] Immediate ping SUCCESS - client registered";
            } else {
            ...
            while (running_.load()) {
                ...
                LOG(INFO)
                    << "[Ping] Sending periodic ping to master_service...";
                auto ping_result = client_.Ping();
                if (ping_result.has_value()) {
                    LOG(INFO) << "[Ping] Ping SUCCESS - connection kept alive";
                } else {
                    LOG(WARNING)
                        << "[Ping] Ping FAILED: error="
                        << static_cast<int>(ping_result.error())
        ...
        while (running_.load()) {
            ...
            // Use timestamp-based key index (incremented from initial
            // timestamp)
            key_index_++;
            ...
            if (exist_result.has_value()) {
                ...
                LOG(WARNING)
                    << "[GET] ExistKey failed for key=" << key
                    << ", error=" << static_cast<int>(exist_result.error());
            }
            ...
            if (!exists) {
                ...
                if (!put_start_result.has_value()) {
                    ...
                    if (err == ErrorCode::NO_AVAILABLE_HANDLE) {
                        LOG(WARNING) << "[PUT] PutStart FAILED: key=" << key
                                     ...
                                     << "(master_service may not have segments "
                                        "configured or is out of space)";
                    } else {
                ...
                auto put_end_result = client_.PutEnd(key, ReplicaType::MEMORY);
                if (put_end_result.has_value()) {
                    ...
                    write_count_++;
                    LOG(ERROR)
                        << "[PUT] PutEnd FAILED: key=" << key << ", error="
                ...
                if (!put_start_result.has_value()) {
                    ...
                    if (err == ErrorCode::NO_AVAILABLE_HANDLE) {
                        LOG(WARNING)
                            << "[PUT] PutStart (update) FAILED: key=" << key
                            << ", error=NO_AVAILABLE_HANDLE (-200) "
                            << "(master_service may not have segments "
                               "configured or is out of space)";
                ...
                if (put_end_result.has_value()) {
                    ...
                    update_count++;
                    write_count_++;
                    // Note: We don't add to written_keys for updates, only for
                    // new keys
            ...
            if (FLAGS_delete_interval > 0 &&
                write_count_ % FLAGS_delete_interval == 0 && write_count_ > 0 &&
                !written_keys.empty()) {
                delete_count++;
                // Delete a random key from the written keys set
                // Use a simple hash-based selection for deterministic behavior
                size_t key_to_delete_index =
                    (write_count_ / FLAGS_delete_interval) %
                    written_keys.size();
                ...
                if (exist_result.has_value()) {
                    ...
                    LOG(WARNING)
                        << "[DELETE] Failed to check key existence: "
                        << delete_key
                        << ", error=" << static_cast<int>(exist_result.error());
                }
                ...
                if (!key_exists) {
                    LOG(INFO)
                        << "[DELETE] Skipping delete: key=" << delete_key
                ...
                if (remove_result.has_value()) {
                    ...
                    LOG(INFO)
                        << "[DELETE] SUCCESS: key=" << delete_key << " deleted";
                } else {
                    ErrorCode err = remove_result.error();
                    if (err == ErrorCode::OBJECT_NOT_FOUND) {
                        // Key was deleted between ExistKey and Remove (race
                        // condition)
                        written_keys.erase(
                            delete_key);  // Remove from tracked keys anyway
                        delete_success_count++;  // Treat as success (key is
                                                 // already gone)
                        LOG(INFO) << "[DELETE] Key " << delete_key
                                  << " was already deleted (race condition)";
                    } else if (err == ErrorCode::OBJECT_HAS_LEASE) {
                        // Object has active lease, cannot delete yet (this is
                        // normal) Note: ExistKey may have granted a lease,
                        // causing this error
                        LOG(INFO) << "[DELETE] Key " << delete_key
                                  << " has active lease, skipping deletion "
                                  << "(this is normal, the key will be deleted "
                                     "after lease expires)";
                        // Don't count as failure, and don't remove from
                        // written_keys so it can be retried later
                    } else {
            ...
            if (total_ops > 0 && total_ops % stats_interval == 0) {
                ...
                double ops_per_sec =
                    (elapsed > 0) ? (total_ops / elapsed) : 0.0;
