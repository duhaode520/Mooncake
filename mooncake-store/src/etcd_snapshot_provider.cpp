...
EtcdSnapshotProvider::EtcdSnapshotProvider(const std::string& etcd_endpoints,
    ...
    backend_ =
        SerializerBackend::Create(SnapshotBackendType::ETCD, etcd_endpoints);
}
...
bool EtcdSnapshotProvider::LoadLatestSnapshot(
    ...
    // But original MasterService code uses SNAPSHOT_ROOT + "/" +
    // SNAPSHOT_LATEST_FILE where SNAPSHOT_ROOT is configurable. Assuming
    // snapshot_root_ passed here is equivalent to MasterService's
    // SNAPSHOT_ROOT.
    (void)
        cluster_id;  // Snapshot path is global in current MasterService layout.
    ...
    if (!backend_->DownloadString(prefix + "manifest.txt", manifest_content)) {
        LOG(ERROR) << "[Standby] Failed to download manifest for "
                   << snapshot_id;
        return false;
    }
    ...
    if (parts.size() >= 10) {
        try {
            ...
            LOG(WARNING)
                << "[Standby] Failed to parse oplog_seq_id from manifest";
        }
    } else {
        LOG(WARNING) << "[Standby] Invalid manifest format (parts="
                     << parts.size() << "), cannot parse oplog_seq_id";
    }
    LOG(INFO) << "[Standby] Snapshot " << snapshot_id
              << " has sequence_id=" << snapshot_sequence_id;

    // 3. Download Metadata
    std::vector<uint8_t> meta_blob;
    if (!backend_->DownloadBuffer(prefix + "metadata", meta_blob)) {
        LOG(ERROR) << "[Standby] Failed to download metadata for "
                   << snapshot_id;
        return false;
    }
    ...
    try {
        oh = msgpack::unpack(reinterpret_cast<const char*>(meta_blob.data()),
                             meta_blob.size());
    } catch (...) {
    ...
    for (size_t i = 0; i < obj.via.map.size; ++i) {
        ...
        if (key == "shards")
            ...
            oplog_seq_obj = &obj.via.map.ptr[i].val;
    }
    ...
    if (oplog_seq_obj) {
        ...
        if (meta_seq_id == 0) {
            if (snapshot_sequence_id > 0) {
                LOG(WARNING) << "[Standby] Metadata oplog_sequence_id is 0; "
                                "keeping manifest seq_id="
                             << snapshot_sequence_id;
            } else {
                LOG(WARNING) << "[Standby] Both manifest seq_id and metadata "
                                "oplog_sequence_id are 0";
            }
        } else {
            if (snapshot_sequence_id > 0 &&
                snapshot_sequence_id != meta_seq_id) {
                LOG(WARNING)
                    << "[Standby] Manifest seq_id (" << snapshot_sequence_id
    ...
    for (size_t i = 0; i < shards_obj->via.map.size; ++i) {
        ...
        try {
            ...
            LOG(ERROR) << "[Standby] Decompression failed for shard " << i
                       << ": " << e.what();
            continue;
        }
        ...
        for (size_t k = 0; k < meta_arr->via.array.size; k += 2) {
            ...
            // Metadata Array Format: [client_id, put_start, size, lease,
            // soft_flag, soft_val, rep_cnt, rep1...] Index:                 0
            // 1          2     3      4          5         6        7...
            if (val_obj.type != msgpack::type::ARRAY ||
                val_obj.via.array.size < 7)
                continue;
            ...
            // When Standby promotes to Master, it usually starts with empty
            // memory/disk locations OR scans local disks. The snapshot metadata
            // mainly tells us "this Key exists and belongs to Client X".
